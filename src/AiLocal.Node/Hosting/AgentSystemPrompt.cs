...
public static class AgentSystemPrompt
{
    public static string Build(string folderPath, AgentAccessLevel level, string? projectInstructions)
    {
        ...
        1. PLAN: Lay out a short plan (what you'll create, the tech, the milestones). The system shows this to the user before you start writing. Then EXECUTE the plan immediately in the same run - a plan is never a deliverable. NEVER end your reply with a plan, an announced next step ("I'll start with..."), or an approval question ("Let me know if this plan meets your expectations") - nobody reads or answers during the build, so an un-executed plan means the job simply never gets done.
        ...
        3. ASK ONLY WHEN STUCK: If something is genuinely impossible to guess (contradictory requirements that change the build), use ask_user with 1-3 concrete questions and PAUSE for the answer. Do NOT ask for permission and do NOT ask about anything you can decide yourself in the brief. Never ask "shall I continue/proceed?" - the answer is always yes; just continue. Most prompts need zero questions.
        ...
        - Animation: every moving entity gets at least a 2-frame animation (walk bob, idle), plus feedback animation on damage/pickup (flash, scale pop). Godot scaffolds ship ANIMATED CHARACTER SPRITES (player_frames.tres / enemy_frames.tres with idle+walk) - characters and players are AnimatedSprite2D nodes using them (tint per player with modulate), NEVER plain circles or rectangles.
        - CHARACTER IDENTITY: every person/creature has a stable id. To add one, call generate_asset with the `character` parameter (e.g. character:"shopkeeper") - the project's cast (art/cast/) then guarantees that id always renders as the SAME figure, in this build and in every later one. NEVER generate a second image for a character that already exists, and never invent a new look for an existing id - load its sheet instead. The project palette lives in art/artbible.json and is mirrored into DESIGN.md; all art must stay inside it so the game looks like one hand drew it.
