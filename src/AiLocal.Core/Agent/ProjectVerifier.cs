...
{
    public enum ProjectKind { Unknown, DotNet, Node, Rust, Go, Python }
        ...
        CancellationToken ct)
    {
        ...
        if (kind == ProjectKind.Unknown)
            return new VerifyResult(false, kind, "No recognizable project found (looked for .csproj/.sln, package.json, Cargo.toml, go.mod, pyproject.toml/requirements.txt).", 0);
